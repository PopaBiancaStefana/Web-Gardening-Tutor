...
async function topUsersTable() {
    return new Promise((resolve, reject) => {
        db.pool.query("select * from (select r.photo_path, r.name,  ROW_NUMBER() over (order by count(c.finished) desc) rownum, count(c.finished) as 'finished_courses' from registered_users r join courses_in_progress c on r.id = c.id_user and c.finished = 1 group by r.name, c.finished, r.photo_path) as q where rownum<=10;", [], (err, result) => {
            if (err) {
                console.log(err);
                reject(err);
                return;
